...
from classifai.servers.jobs import job_manager, STATUS_PROCESSING, STATUS_COMPLETED, STATUS_FAILED
...
def process_batch_job(
    ...
    try:
        # Calcular total de líneas (aprox) para medir progreso
        total_rows = sum(1 for _ in open(input_filepath, 'r', encoding="utf-8")) - 1
        if total_rows <= 0:
            job_manager.update_status(job_id, status=STATUS_FAILED, message="El archivo CSV está vacío o sin registros")
            return

        total_chunks = math.ceil(total_rows / CHUNK_SIZE)
        job_manager.update_status(job_id, status=STATUS_PROCESSING, message=f"Total a procesar: {total_rows} filas en {total_chunks} lotes")
        ...
        for i, chunk in enumerate(pd.read_csv(input_filepath, chunksize=CHUNK_SIZE, dtype=str, encoding="utf-8")):
            ...
            id_col = None
            if "id_registro" in cols: id_col = "id_registro"
            ...
            literal_col = None
            if "literal" in cols: literal_col = "literal"
            elif "text" in cols: literal_col = "text"
            elif "descripcion" in cols: literal_col = "descripcion"
            else:
            ...
            new_cols = {
                "codigo_1": [], "descripcion_1": [], "prob_1": [],
                "codigo_2": [], "descripcion_2": [], "prob_2": [],
                "codigo_3": [], "descripcion_3": [], "prob_3": []
            }
            ...
            for _, row in chunk.iterrows():
                ...
                for k in new_cols.keys():
                    new_cols[k].append(r_data.get(k, ""))
