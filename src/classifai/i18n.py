        ...
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            if reader.fieldnames:
                for col in reader.fieldnames:
                    col_lower = col.lower()
                    if col_lower == "id" or col_lower == "id_clasificacion":
                        id_col = col
                    if col_lower == "text" or col_lower == "descripcion":
                        text_col = col
...
def get_description(api_endpoint_name: str, code: str, lang: str = "es") -> Optional[str]:
    """
    ...
    if base in _i18n_cache and lang in _i18n_cache[base]:
        if code_normalized in _i18n_cache[base][lang]:
            return _i18n_cache[base][lang][code_normalized]
            
    # Fallback 1: Buscar en español ("es") por defecto LAC
    if base in _i18n_cache and "es" in _i18n_cache[base]:
        if code_normalized in _i18n_cache[base]["es"]:
            return _i18n_cache[base]["es"][code_normalized]

    # Fallback 2: Buscar en CUALQUIER idioma disponible
    if base in _i18n_cache:
        for available_lang, dct in _i18n_cache[base].items():
